...
typedef enum lt_ret_t {
    ...
    */
    ...
    /** @brief Error: R-Memory write failed (chip rejected, slot not erased).
     * @details Appeneded at the end of the enum (after LT_NONCE_OVERFLOW) to preserve
     * existing enum values — the C++ wrapper references this by name, not by integer value.
     * LT_RET_T_LAST_VALUE bumped accordingly. */
    LT_L3_R_MEM_DATA_WRITE_FAILED = 47,
    /** @brief Error: R-Memory erase failed (chip rejected the operation). */
    LT_L3_R_MEM_DATA_ERASE_FAILED = 48,

    /** @brief Special helper value used to signalize the last enum value, used in lt_ret_verbose. */
    LT_RET_T_LAST_VALUE = 49
} lt_ret_t;
