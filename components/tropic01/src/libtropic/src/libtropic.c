...
static const char *lt_ret_strs[] = {"LT_OK",
                                    ...
                                    "LT_NONCE_OVERFLOW",
                                    "LT_L3_R_MEM_DATA_WRITE_FAILED",
                                    "LT_L3_R_MEM_DATA_ERASE_FAILED"};
                                                 ...
                                                 const lt_pkey_index_t pkey_index)
{
    ...
    // NOTE: cert buffers heap-allocated (4×700B on stack overflows ESPHome's task).
    uint8_t *cert_ese = (uint8_t *)calloc(1, TR01_L2_GET_INFO_REQ_CERT_SIZE_SINGLE);
    uint8_t *cert_xxxx = (uint8_t *)calloc(1, TR01_L2_GET_INFO_REQ_CERT_SIZE_SINGLE);
    uint8_t *cert_tr01 = (uint8_t *)calloc(1, TR01_L2_GET_INFO_REQ_CERT_SIZE_SINGLE);
    uint8_t *cert_root = (uint8_t *)calloc(1, TR01_L2_GET_INFO_REQ_CERT_SIZE_SINGLE);

    if (!cert_ese || !cert_xxxx || !cert_tr01 || !cert_root) {
        free(cert_ese);
        free(cert_xxxx);
        free(cert_tr01);
        free(cert_root);
