...
bool Tropic01Component::init_tropic() {
  ...
  // v4.0.0 lt_init() auto-reboots from MAINTENANCE to APPLICATION on first boot.
  // If the reboot was unsuccessful (e.g. invalid FW), the chip stays in
  // MAINTENANCE mode and lt_init() still returns LT_OK.
  // Check and warn, but don't block — FW update via bootloader is still possible.
  lt_tr01_mode_t mode;
  ret = lt_get_tr01_mode(handle_, &mode);
  if (ret == LT_OK && mode != LT_TR01_APPLICATION) {
    ESP_LOGW(TAG, "TROPIC01 in %s mode after init — App FW may be missing or invalid",
             mode_to_str(tr01_mode_to_chip(mode)));
    ESP_LOGW(TAG, "  FW update required before secure features (R-Memory, ECC) are available");
...
void Tropic01Component::update_sensors() {
  ...
  if (mode == ChipMode::ALARM) {
    ...
    if (default_key_invalid_ && !lockout_warned_) {
      ...
      ESP_LOGW(TAG, "TROPIC01 in MAINTENANCE mode — App FW not loaded");
    }
    this->status_set_warning();
  } else {
